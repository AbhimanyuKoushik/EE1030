...
void parab_x2_4ay_gen(FILE *fptr, double **x1, double **x2, double a, int num_points) {
    
    double tinit = (x1[0][0] / (2 * a));
    double tfinal = (x2[0][0] / (2 * a));
...
double area(double (*func)(double), double lower_limit, double upper_limit)
{
    ...
    {
        //sum += (1.0/2.0)*(func(i)+func(i-1e-3))* 1e-3;
        sum += func(i)*1e-3;
    }
    return sum;
}
...
int main(){
    ...
    parab_x2_4ay_gen(fptr,x1,x2,-u[1][0]/2,200);
