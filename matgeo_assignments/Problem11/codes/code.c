...
double** sidelength_vector_gen_2anglesBCsideBClength(double angleB, double angleC, double sideBC) {
    ...
    b[1][0] = 1;
    b[2][0] = 0;
    sidematrix = Matscale(b, 3, 1, sideBC);
...
int main() {
    ...
    length = sidelength_vector_gen_2anglesBCsideBClength(M_PI/4, M_PI/6, 7);
    sideBC = length[0][0];
    sideCA = length[1][0];
    sideAB = length[2][0];

    // Generate triangle points
    twoDtriangle_gen(sideAB, sideBC, sideCA, "triangle1.dat");
    
    sideBC = 3*length[0][0]/4;
    sideCA = 3*length[1][0]/4;
    sideAB = 3*length[2][0]/4;
    
    twoDtriangle_gen(sideAB, sideBC, sideCA, "triangle2.dat");
